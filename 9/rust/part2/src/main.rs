...
fn main() {
    ...
    visited.insert(position_to_key(&knots.last().unwrap()));
    ...
    loop {
        ...
        knots[0] = knots[0].moved(head_direction);
        head_count -= 1;
        if head_count == 0 {
        ...
        for i in 1..knots.len() {
            if !knots[i].is_adjacent_to(&knots[i - 1]) {
                knots[i] = knots[i].moved_towards(&knots[i - 1]);
            }
            visited.insert(position_to_key(&knots.last().unwrap()));
        }
    }
